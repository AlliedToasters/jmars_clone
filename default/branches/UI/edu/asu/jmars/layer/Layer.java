       ...
       private class Light extends JTextField
		{
			...
			protected void paintComponent(Graphics g) {
				...
				Color bgColor = parse(((ThemePanel)GUITheme.get("panel")).getBackground());
				if(getParent()!=null && getParent().getBackground()!=null){
					bgColor = getParent().getBackground();
				}
				//set the background color to match that of the current theme
				g.setColor(bgColor);
				g.fillRect(0, 0, getWidth(), getHeight());
				//draw the status indicator as a circle
