...
public class TriangleMesh implements GLRenderable, Disposable {
	...
	private float[] texture;		// texture coords for the entire mesh
    ...
    public TriangleMesh(String meshFile, Ellipsoid body, String meshName) throws IOException, URISyntaxException {
		...
		this.realizeOcttree();
    	texture = new float[vertices.length / 2 * 3];
    	Arrays.fill(this.texture, Float.NaN);
		ThreeDManager.getInstance().switchLight(true);
    }
	...
	private void realizeOcttree() {
		...
		for (int i=0; i<indices.length; i+=3) {
			...
			if (facetIds != null && facetIds.length > j) {
				t.id = facetIds[j];
			} else {
				t.id = j;	
			}
