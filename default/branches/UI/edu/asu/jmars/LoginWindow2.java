...
public class LoginWindow2 extends JFrame {
	...
	private void buildUI(){
		...
		Color themecolor = parse(ThemeProvider.getInstance().getAction().getMain());
		Color imgColor = ((ThemeImages) GUITheme.get("images")).getFill();		
		Color textColor = parse(((ThemeText) GUITheme.get("text")).getTextcolor());
	...
	class UpdateButton extends JButton {
        ...
        private Color normalColor = parse(((ThemeButton)GUITheme.get("button")).getDefaultback());              
        private Color hoverColor = parse(((ThemeButton)GUITheme.get("button")).getHighlight());
        private Color foregroundColor = parse(((ThemeButton)GUITheme.get("button")).getDefaultforeground());
	...
	class RSSPanel extends JPanel {
		...
		private void buildUI(){
			...
			Color themecolor = parse(((ThemeButton)GUITheme.get("button")).getDefaultback());
			Color primaryback = parse(((ThemePanel)GUITheme.get("panel")).getBackground());
